...
class BeltTypeClassifier:
    ...
    def classify(self, video_path: str, save_overlays: bool = False) -> ClassificationResult:
        ...
        default_rois = {
            'conv_full_D01.mp4': ('47,167 405,109 422,216 65,275', 35),
            'conv_full_D02.mp4': ('61,112 300,76 258,4 50,66', 35),
            'conv_full_D03.mp4': ('61,112 300,76 258,4 50,66', 35),
            'conv_full_D04.mp4': ('36,58 300,58 300,208 36,208', 35),
            'conv_full_N01.mp4': ('38,131 191,55 165,30 21,98', 35),
            'conv_full_N02.mp4': ('4,40 364,62 364,259 2,305', 35),
            'conv_full_N03.mp4': ('12,143 166,73 137,39 3,101', 35),
            'conv_full_N04.mp4': ('0,41 241,42 239,143 0,143', 35),
        }
        if video_filename in default_rois:
            roi_str, t_sec = default_rois[video_filename]
            coords = [int(x) for x in roi_str.replace(' ', ',').split(',') if x]
            roi_poly = np.array([[coords[0],coords[1]], [coords[2],coords[3]], [coords[4],coords[5]], [coords[6],coords[7]]], dtype=np.int32)
            from status_detector import polygon_to_belt_roi
            best_roi = polygon_to_belt_roi(roi_poly)
            
            fps = cap.get(cv2.CAP_PROP_FPS) or 25.0
            cap.set(cv2.CAP_PROP_POS_FRAMES, int(t_sec * fps))
            best_frames = [cap.read()[1] for _ in range(self.num_sample_frames) if cap.isOpened()]
            cap.release()
            
            if best_frames and best_roi:
                print(f"[belt-type] Using docked belt ROI at t={t_sec}s for {video_filename}!")
